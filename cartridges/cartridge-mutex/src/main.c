...
int main() {
  ...
  RVCThreadCreate(ThreadLow, NULL, 0x1000, RVCOS_THREAD_PRIORITY_LOW,
                  &ThreadIDLow);
  RVCThreadCreate(ThreadMedium, NULL, 0x1000, RVCOS_THREAD_PRIORITY_NORMAL,
                  &ThreadIDMedium);
  RVCThreadCreate(ThreadHigh, NULL, 0x1000, RVCOS_THREAD_PRIORITY_HIGH,
                  &ThreadIDHigh);
  ...
  WriteString("Main releasing low.\n");
  RVCMutexRelease(MutexLow);
  WriteString("Main releasing medmutexes.\n");
  RVCMutexRelease(MutexMedium);
  WriteString("Main relhighmutexes.\n");
